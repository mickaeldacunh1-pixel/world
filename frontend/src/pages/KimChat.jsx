...
export default function KimChat() {
  ...
  const [sessionId, setSessionId] = useState(() => localStorage.getItem('kim_session_id') || null);
  ...
  const sendMessage = async (text) => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/kim/chat`, {
        method: 'POST',
      ...
      if (data.session_id) {
        ...
        localStorage.setItem('kim_session_id', data.session_id);
      }
  ...
  const newChat = () => {
    setMessages([]);
    setSessionId(null);
    localStorage.removeItem('kim_session_id');
  };
