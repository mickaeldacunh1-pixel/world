...
function DraggableElement({ 
  ...
  const renderContent = () => {
    ...
    switch (element.id) {
      ...
      case 'diag_button':
        return (
          <button className={`${isMobileView ? 'px-2 py-1 text-[10px]' : 'px-3 py-2 text-xs'} bg-emerald-500/80 text-white rounded-lg font-medium`}>
            🩺 {isMobileView ? 'Diag' : 'Diagnostic IA'}
          </button>
        );
      case 'radio_button':
        return (
